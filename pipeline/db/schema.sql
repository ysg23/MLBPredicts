...
-- ============================================================
-- PHASE 1A: SCORE RUN AUDIT + LINEUPS + FEATURE STORE
-- ============================================================
...
-- ============================================================
-- MULTI-MARKET GENERIC TABLES (HR, K, ML, TOTAL, F5, TT, etc.)
-- ============================================================
...
CREATE TABLE IF NOT EXISTS market_odds (
    ...
    game_id INTEGER REFERENCES games(game_id),
    event_id TEXT,
    market TEXT NOT NULL,                      -- 'HR','K','ML','TOTAL','F5_ML','F5_TOTAL','TEAM_TOTAL', etc.
    entity_type TEXT CHECK(entity_type IN ('batter', 'pitcher', 'team', 'game')),
    player_id INTEGER,                         -- nullable for team/game markets
    team_id TEXT,
    opponent_team_id TEXT,
    team_abbr TEXT,
    ...
    selection_key TEXT,
    side TEXT CHECK(side IN ('OVER', 'UNDER', 'YES', 'NO', 'HOME', 'AWAY')),
    bet_type TEXT NOT NULL,                    -- e.g. 'HR_1PLUS', 'K_OVER', 'ML', 'TOTAL_OVER', 'F5_ML'
    line REAL,                                 -- nullable for markets without a line
    price_american INTEGER,
    price_decimal REAL,
    implied_probability REAL,
    odds_decimal REAL,                         -- backward compatible alias for decimal odds
    sportsbook TEXT NOT NULL,
    source_market_key TEXT,
    is_best_available INTEGER NOT NULL DEFAULT 0 CHECK(is_best_available IN (0, 1)),
...
CREATE TABLE IF NOT EXISTS market_outcomes (
    ...
    line REAL,
    outcome_value REAL,                        -- numeric result (e.g. Ks, runs, 1/0)
    outcome_text TEXT,                         -- optional label
    settled_at DATETIME,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS model_scores (
    ...
    bet_type TEXT NOT NULL,
    line REAL,
    model_score REAL NOT NULL,                 -- 0-100 composite
    model_prob REAL,                           -- 0-1 probability (if applicable)
    model_projection REAL,                     -- projection (e.g. Ks, total runs)
    book_implied_prob REAL,                    -- 0-1 (if odds available)
    edge REAL,                                 -- model_prob - implied_prob (or other edge metric)
    signal TEXT CHECK(signal IN ('BET','LEAN','SKIP','FADE')),
    confidence_band TEXT CHECK(confidence_band IN ('HIGH', 'MEDIUM', 'LOW')),
    factors_json TEXT,                         -- JSON string of factor breakdown
    reasons_json TEXT,
    risk_flags_json TEXT,
    lineup_confirmed INTEGER CHECK(lineup_confirmed IN (0, 1)),
    weather_final INTEGER CHECK(weather_final IN (0, 1)),
