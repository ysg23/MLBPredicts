...
def _starter_strength(pitcher: dict[str, Any] | None) -> float:
    ...
    return ((k - bb) * 0.9) - ((hr9 - 1.1) * 14.0) + ((role - 0.6) * 10.0)
...
def _offense_strength(team: dict[str, Any] | None) -> float:
    ...
    return ((runs - 4.4) * 2.8) + ((obp - 0.320) * 120.0) + ((slg - 0.405) * 55.0)
...
def score_game(game: GameContext, weather: dict | None, park_factor: float, season: int) -> list[dict]:
    ...
    home_strength = _offense_strength(home_team) + (_starter_strength(home_pitcher) * 1.15)
    away_strength = _offense_strength(away_team) + (_starter_strength(away_pitcher) * 1.15)
    f5_home_field_adv = 0.9
    weather_home_adj = (weather_multiplier - 1.0) * 1.2
    net_home = home_strength - away_strength + f5_home_field_adv + weather_home_adj
    ...
    for odds in odds_rows:
        ...
        model_score = 50.0 + ((model_prob - 0.5) * 95.0) + ((side_strength - opp_strength) * 0.35)
        if edge_pct is not None:
            model_score += max(-8.0, min(8.0, edge_pct * 0.35))
        model_score = max(0.0, min(100.0, model_score))

        factors = {
            "starter_edge_score": max(0.0, min(100.0, 50.0 + ((_starter_strength(home_pitcher if side == "HOME" else away_pitcher) - _starter_strength(away_pitcher if side == "HOME" else home_pitcher)) * 2.3))),
            "offense_edge_score": max(0.0, min(100.0, 50.0 + ((_offense_strength(home_team if side == "HOME" else away_team) - _offense_strength(away_team if side == "HOME" else home_team)) * 2.8))),
            "f5_home_field_score": 58.0 if side == "HOME" else 42.0,
            "weather_context_score": max(0.0, min(100.0, 50.0 + ((weather_multiplier - 1.0) * 140.0))),
        }
