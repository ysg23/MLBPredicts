...
def score_game(game: GameContext, weather: dict | None, park_factor: float, season: int) -> list[dict]:
    ...
    # Determine the line: from odds if available, otherwise round projection to nearest 0.5
    default_line = round(total_projection * 2.0) / 2.0
    ...
    for side in sides_to_score:
        ...
        edge_proj = projection_edge_pct(total_projection, line) if odds else None
        ...
        results.append(
            {
                ...
                "side": side,
                "bet_type": (odds.get("bet_type") if odds else None) or f"TOTAL_{side}",
                "line": line,
